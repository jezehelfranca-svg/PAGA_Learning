(function initializeSoundCoverageApp() {
  ...
  function renderNoiseInspector(zone) {
    const effectiveTarget = Model.targetForNoiseZone(project, zone);
    inspector.innerHTML = `
      <form class="inspector-form" autocomplete="off">
        ...
        <div class="provenance-note sourced"><b>Effective zone target: ${round(effectiveTarget, 1)} ${escapeHtml(decibelUnit())}.</b> The zone ambient replaces the project ambient inside this rectangle, even when it is lower. Target is the greater of ambient plus margin or the applicable minimum. When zones overlap, the most demanding active zone target is used.</div>
  ...
  function updateSelectedField(control) {
    ...
    markChanged({ refreshInspector: control.type === "checkbox" || refreshNoiseTarget });
