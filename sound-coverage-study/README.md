...
- Single-source tap edits persist; entering a tap above rated power raises the rating with a visible notice, while lowering rated power reconciles the tap. Batch fields show current or mixed state and explicitly validate tap against rated power.
- Two independently editable project-wide equipment thresholds: weatherproof loudspeakers (default 124 dB(A) at 1 m) and flameproof / hazardous-area loudspeakers (default 119 dB(A) at 1 m), with a project default source class and optional per-source or batch classification overrides evaluated at rated power.
